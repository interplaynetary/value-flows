Repository
See the Data Repositories Guide for a higher-level introduction.
...
Repositories and their contents are canonically stored in DRISL CBOR, as a graph of data objects referencing each other by content hash (CID Links). Large binary blobs are not stored directly in repositories, though they are referenced by hash (CID). This includes images and other media objects. Repositories can be exported as CAR files for offline backup, account migration, or other purposes.
...
Repository Diffs
An updated version of the Synchronization protocol is being rolled out to the live network in early 2025. There is a "Sync v1.1" proposal document and an update blog post with deployment details. The written specifications will be updated soon.
